...
FROM (
    SELECT 'sys:student:import' AS permission_code, '导入学员' AS permission_name, 'POST' AS api_method, '/api/v1/admin/students/import' AS api_path, 46 AS sort_order
    UNION ALL
    SELECT 'sys:student:export', '导出学员', 'GET', '/api/v1/admin/students/export', 47
) permission_seed
WHERE NOT EXISTS (
    SELECT 1
...
JOIN sys_permissions permission_table
    ON permission_table.permission_code IN ('sys:student:import', 'sys:student:export')
