...
- `[~]` 收敛官网专题页页面化契约：消除 `AppTopicItemResponse.resource` 裸 `Object` 返回，明确专题标签、详情分区、卡片 DTO 与“更多”分页接口 schema。
- `[x]` 导出最新 `api/api.json`。
...
- `[~]` 官网专题页第一轮审计已完成：当前用户端专题接口已具备列表和详情能力，但详情仅返回平铺 `items` 与 `Object resource`，缺少页面所需的“学习 / 视频 / 音频”分区语义、专题卡片主标签、分区“更多”分页能力和稳定 OpenAPI 类型定义。
- `[~]` 管理端联调第二批反馈待收口：学员导入/导出接口已补齐并进入 Swagger 契约；审核日志已有统一接口但缺少资源类型枚举、审核人展示名与状态语义说明；图书考卷配置、专题分项规则、专家分类层级、首页内容 `contentType/targetId` 规则、反馈字段语义、答疑状态输出形式、测试资源地址策略仍需明确契约或补实现。
...
- `[~]` 官网专题页补齐：专题列表补展示标签；专题详情按页面结构补“学习 / 视频 / 音频”分区、每区首屏条数和“更多”跳转所需分页接口；专题卡片统一返回页面可直接渲染的标题、副标题、封面、标签和浏览/收藏统计。
...
- `[~]` 管理端联调第二批差距补齐：
  ...
  3. 审核日志：已存在统一接口 `GET /api/v1/admin/system/audit-records`，并且课程/图书/播客/专题/直播审核时都会写审计记录；但当前仅返回 `auditorId`、`beforeStatus`、`afterStatus`，缺少审核人展示信息、资源类型枚举说明和状态中文语义，前端仍难直接接入“审核日志”按钮。
  4. 图书考卷配置：当前图书 DTO 已包含 `paperId`，说明代码侧是“图书级单考卷”模型；但缺少独立“考卷配置”操作文档和页面级动作说明，需明确是复用图书新增/修改接口还是补专门配置接口。
  5. 专题分项配置：当前仅有统一 `PUT /api/v1/admin/content/topics/{id}/items`，且代码侧未校验 `itemType` 枚举、允许资源范围、排序/去重规则或按类型读取能力，仍不足以支撑手册里的分项配置入口。
...
1. 收口审核日志、图书考卷配置、专题分项规则、专家分类层级、首页内容快捷配置等“已有部分能力但契约不清”的管理端联调项。
