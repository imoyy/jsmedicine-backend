...
- `[~]` 管理端联调第二批反馈待收口：学员导入/导出接口已补齐并进入 Swagger 契约；审核日志响应已补资源类型说明、审核人展示名与状态语义字段；图书考卷配置、专题分项规则、专家分类层级、首页内容 `contentType/targetId` 规则、反馈字段语义、答疑状态输出形式、测试资源地址策略仍需明确契约或补实现。
...
1. 收口图书考卷配置、专题分项规则、专家分类层级、首页内容快捷配置等“已有部分能力但契约不清”的管理端联调项。
